...
*/
 ...
 bool Bus_Parallel16::init(void)
 {
    ...
    LCD_CAM.lcd_clock.lcd_clk_sel = 2;        // Use main clock / 2 (240 MHz when full speed, which it should be)
    ...
    LCD_CAM.lcd_misc.lcd_bk_en = 0;          // https://esp32.com/viewtopic.php?t=24459&start=60#p91835
    ...
    for(int i = 0; i < 16; i++)
    {
      if (pins[i] >= 0) { // -1 value will CRASH the ESP32!
        ...
        gpio_set_drive_capability((gpio_num_t)pins[i], MTX_HUB75_DRIVE_CAP);
      }
    }
...
*/
    // Clock
      ...
      gpio_set_drive_capability((gpio_num_t)_cfg.pin_wr, MTX_HUB75_DRIVE_CAP);
